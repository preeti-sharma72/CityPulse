...
function logUpstream(name, url) {
  upstreamCallCount += 1;
  console.info(`[upstream] ${name} call #${upstreamCallCount} ${url}`);
}

function normalize(sourceId, locationId, metricId, value, unit, timestamp) {
  if (value === undefined || value === null || Number.isNaN(Number(value))) return null;
  return { source_id: sourceId, location_id: locationId, metric_id: metricId, value: Number(value), unit, timestamp_utc: new Date(timestamp || Date.now()).toISOString() };
}
...
async function getAqi() {
  ...
  if (process.env.WAQI_TOKEN && !process.env.WAQI_TOKEN.startsWith('replace_')) {
    ...
    try {
      ...
      return { source: 'waqi', readings: dedupe([normalize('waqi', locationId, 'aqi', data.data.aqi, 'AQI', timestamp), normalize('waqi', locationId, 'pm2.5', iaqi.pm25?.v, 'µg/m³', timestamp), normalize('waqi', locationId, 'pm10', iaqi.pm10?.v, 'µg/m³', timestamp)]), observed_at: timestamp };
    } catch (error) {
      console.warn(`[cache] waqi unavailable, using Open-Meteo: ${error.message}`);
      return getOpenMeteoAqi(lat, lon, locationId);
    }
  }
  if (!process.env.OPENAQ_API_KEY || process.env.OPENAQ_API_KEY.startsWith('replace_')) return getOpenMeteoAqi(lat, lon, locationId);
  ...
  let data;
  try {
    data = await fetchWithRetry(url, { name: 'openaq', headers });
  } catch (error) {
    console.warn(`[cache] openaq unavailable, using Open-Meteo: ${error.message}`);
    return getOpenMeteoAqi(lat, lon, locationId);
  }
  const measurements = data.results || [];
  return { source: 'openaq', readings: dedupe(measurements.map(measurement => normalize('openaq', locationId, measurement.parameter?.name || measurement.parameter, measurement.value, measurement.unit, measurement.datetime?.utc || timestamp))), observed_at: timestamp };
}

async function getOpenMeteoAqi(lat, lon, locationId) {
  const url = `https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${encodeURIComponent(lat)}&longitude=${encodeURIComponent(lon)}&current=us_aqi,pm2_5,pm10&timezone=UTC`;
  const data = await fetchWithRetry(url, { name: 'open-meteo-aqi' });
  const current = data.current || {};
  const timestamp = current.time ? new Date(current.time).toISOString() : new Date().toISOString();
  return { source: 'open-meteo', readings: dedupe([
    normalize('open-meteo', locationId, 'aqi', current.us_aqi, 'AQI', timestamp),
    normalize('open-meteo', locationId, 'pm2.5', current.pm2_5, 'µg/m³', timestamp),
    normalize('open-meteo', locationId, 'pm10', current.pm10, 'µg/m³', timestamp)
